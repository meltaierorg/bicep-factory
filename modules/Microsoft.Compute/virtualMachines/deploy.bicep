...
param deployASG bool = false
param ASGPrefix string = 'default'
var ASGName = 'asg-${ASGPrefix}'
  ...
  properties: {
    ...
    storageProfile: {
      ...
      dataDisks: [for (dataDisk, index) in dataDisks: {
        lun: index
        name: '${name}_datadisk_${padLeft((index + 1), 2, '0')}'
    ...
    networkProfile: {
      networkInterfaces: [for (nicConfiguration, index) in nicConfigurations: {
        ...
        id: az.resourceId('Microsoft.Network/networkInterfaces', 'nic-${name}')
      }]
...
module vm_KeyvaultExtension 'extensions/deploy.bicep' = if (extensionKeyvaultConfig.enabled) {
  name: '${uniqueString(deployment().name, location)}-VM-Keyvault-Connect'
  params: {
    virtualMachineName: vm.name
    name: 'KeyVaultForWindows'
    publisher: 'Microsoft.Azure.KeyVault'
    type: 'KeyVaultForWindows'
    typeHandlerVersion: contains(extensionKeyvaultConfig, 'typeHandlerVersion') ? extensionKeyvaultConfig.typeHandlerVersion : '3.0'
    autoUpgradeMinorVersion: contains(extensionKeyvaultConfig, 'autoUpgradeMinorVersion') ? extensionKeyvaultConfig.autoUpgradeMinorVersion : true
    enableAutomaticUpgrade: contains(extensionKeyvaultConfig, 'enableAutomaticUpgrade') ? extensionKeyvaultConfig.enableAutomaticUpgrade : false
    settings: extensionKeyvaultConfig.settings
    location: location
  }
}
